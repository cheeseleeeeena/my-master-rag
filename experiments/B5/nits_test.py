...
if __name__ == "__main__":
    ...
    for split in ["test", "validation"]:
        ...
        # dataset.to_json(f"{output_dir}/{split}.json")

        for paper in dataset:
            section_names, paras_in_section = paper["full_text"].values()
            # print(section_names[0])
            # print(paras_in_section[0])
            qids = paper["qas"]["question_id"]
            grouped_answers = paper["qas"]["answers"]
            evidences_by_question = {}
            for q_id, q_answers in zip(qids, grouped_answers):
                # for annotator in q_answers["answer"]:
                evidences_by_question[q_id] = [
                    annotator["highlighted_evidence"]
                    for annotator in q_answers["answer"]
                ]
                print(q_id)
                print(len(evidences_by_question[q_id]))
                break
            break
        break
